package com.tmax.openframe.variable.group;

import java.math.*;
import java.util.*;
import lombok.*;
import org.apache.commons.lang3.StringUtils;

/** 
 * <p>This source code was generated by T-Up OpenFrame COBOL to Java migrator</p>
 * <p>  Generation date : 2024/03/20 15:15:10 KST</p>
 * <hr>
 * <p>Class that defines variables for COBOL group item FD-TRANFILE-REC</p>
 */
public @AllArgsConstructor @NoArgsConstructor class Cbtrn01cFdTranfileRec {
    // [T-Up#INFO][CBTRN01C.cbl:93] FD-TRANS-ID (level: 5)
    private @Getter String fdTransId = StringUtils.repeat(' ', 16);
    // [T-Up#INFO][CBTRN01C.cbl:94] FD-ACCT-DATA (level: 5)
    private @Getter String fdAcctData = StringUtils.repeat(' ', 334);

    public void set(String source) {
        source = StringUtils.truncate(StringUtils.rightPad(source, 350), 350);
        setFdTransId(source.substring(0, 16));
        setFdAcctData(source.substring(16, 350));
    }

    @Override
    public String toString() {
        StringBuilder sb = new StringBuilder();
        sb.append(getFdTransId());
        sb.append(getFdAcctData());
        return sb.toString();
    }

    public void setFdTransId(String source) {
        fdTransId = StringUtils.truncate(StringUtils.rightPad(source, 16), 16);
    }

    public void setFdAcctData(String source) {
        fdAcctData = StringUtils.truncate(StringUtils.rightPad(source, 334),
                334);
    }
}
